...
export type GlyphKey = 'social' | 'pen' | 'film' | 'website' | 'author' | 'blog'
...
function branch(side: 1 | -1, id: string) {
  const n = 11
  const a0 = 34
  const a1 = 168
...
function TileMark({ x, y, children }: { x: number; y: number; children: React.ReactNode }) {
  return (
...
export default function OfferEmblem({ glyph, className }: { glyph: GlyphKey; className?: string }) {
...
export function BookOrnament({ size = 30 }: { size?: number }) {
  return (

...
export default function BookLongPage({ book, kind, quote, related }: { book: BookView; kind: 'review' | 'summary'; quote: { text: string; author: string }; related: BookView['related'] }) {
  ...
  // Only the facts this book actually has. A row with nothing to show would
  // otherwise print its label and icon above an empty line.
  const metaRows = [
  ...
  // Whether the long-form article has anything in it at all. An editor who
  // has not written this page yet gets no empty card with a heading over it.
  const hasArticle =
    hasList(data.intro) || hasList(data.sections) || has(data.quote) ||
    (isReview
      ? hasList(data.worked) || hasList(data.better) || has(data.verdict) ||
        hasNum(book.review.overall) || hasList(data.bars)
      : hasList(data.takeaways))
  ...
  return (
    <div className="bp bp-long">
      ...
      <main className="bp-main">
        ...
        {/* ---- book identity strip ---- */}
        <section className={`bp-card bp-lhero${has(book.coverSrc) ? '' : ' is-textonly'}`}>
          {has(book.coverSrc) && (
          ...
          <div className="bp-lhero-info">
            ...
            <div className="bp-lhero-cta">
              <a className="bp-btn bp-btn-gold bp-btn-lg" href={`/books/${book.slug}`}>
                Back to Book
                <I.back width="17" height="17" fill="none" stroke="currentColor" strokeWidth="2"
                  strokeLinecap="round" strokeLinejoin="round" />
              </a>
              <a className="bp-btn bp-btn-ghost bp-btn-lg" href={`/books/${book.slug}`}>
                Book Details
        ...
        <div className="bp-grid">
          ...
          <div className="bp-col">
            ...
            <article className="bp-card bp-article">
              ...
              {isReview
                ? data.intro.map((p: any, i: number) => <p className="bp-lede" key={i}>{p}</p>)
                : hasList(data.intro) && <p className="bp-lede">{data.intro}</p>}

              {data.sections.map((s: any, i: number) => (
                <section className="bp-article-sec" key={s.title || i}>
                  {has(s.title) && <h3>{s.title}</h3>}
                  {has(s.body) && <p>{s.body}</p>}
                </section>
              ))}
              ...
              {isReview
                ? (hasNum(book.review.overall) || hasList(data.bars)) && (
                    <section className={`bp-scorecard${hasNum(book.review.overall) ? '' : ' is-barsonly'}`}>
                    ...
                    <section className="bp-takeaways">
                      ...
                      <div className="bp-takeaway-grid">
                        {data.takeaways.map((t: any) => {
                          const Ico = I[t.icon as keyof typeof I] || I.book
                          return (
                            <article key={t.title}>
                              <Ico width="24" height="24" fill="none" stroke="#c9962f" strokeWidth="1.5"
                                strokeLinecap="round" strokeLinejoin="round" />
                              {has(t.title) && <h4>{t.title}</h4>}
                              {has(t.text) && <p>{t.text}</p>}
                            </article>
                          )
                        })}
                      </div>
          ...
          <div className="bp-col bp-col-aside">
            ...
            {hasList(book.videos) && (
              <section className="bp-card bp-watch">
                  ...
                  {book.videos.map((v: any) => (
                    <li key={v.label}>
                      ...
                      <span className="bp-watch-meta">
                        {has(v.label) && <strong>{v.label}</strong>}
                ...
                {!isReview && (
                  <a className="bp-all-retailers" href={`/books/${book.slug}`}>
                    View all videos
              ...
              <section className="bp-card bp-more">
                ...
                <ul>
                  {related.map((r: any) => (
                    <li key={r.slug}>
                      <a href={`/books/${r.slug}`}>
                        {has(r.coverSrc) && <img src={r.coverSrc} alt="" loading="lazy" />}
                        <span>
                          <strong>{r.title}</strong>
                          {has(r.author) && <em>{r.author}</em>}
                          {hasNum(r.rating) && (

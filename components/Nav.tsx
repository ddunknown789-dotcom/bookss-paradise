...
export type NavProps = { links: NavLink[]; joinLabel?: string; joinHref?: string }
...
export default function Nav({ links, joinLabel = 'Join Us', joinHref = '#newsletter' }: NavProps) {
  const LINKS = links
    ...
    >
      <div className="nav-inner">
        ...
        <div className="nav-right">
          ...
          <a className="btn btn-gold btn-join" href={joinHref}>{joinLabel}</a>
        </div>
        <button
      ...
      <div className="nav-drawer" role="dialog" aria-modal="true" aria-hidden={!menuOpen}>
        ...
        <a className="btn btn-gold btn-join-drawer" href={joinHref} onClick={() => setMenuOpen(false)}>
          {joinLabel}
        </a>
      </div>
    </header>

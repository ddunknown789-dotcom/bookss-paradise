...
/* ---------------------------------------------------------------------------
   ...
   The assistant used to import the site's books/interviews/weeks directly.
   Those now live in Supabase, so the page hands them in once on mount and BP
   answers from exactly what is published — nothing can go stale.
   ------------------------------------------------------------------------- */
...
const WEEKS_HREF = '/books-of-the-week'
const bookHref = (b) => b.href || `/books/${b.slug}`
const interviewHref = (i) => i.href || `/interviews/${i.slug}`

export function setCatalog({ books = [], interviews = [], week = null } = {}) {
  BOOKS = books
...
let NEWEST = []
let TOP_RATED = []
let NEWEST_INTERVIEWS = []
let ALL_GENRES = []

function recompute() {
  NEWEST = [...BOOKS].sort((a, b) => publishedAt(b) - publishedAt(a))
  TOP_RATED = [...BOOKS].sort((a, b) => b.rating - a.rating)
  NEWEST_INTERVIEWS = [...INTERVIEWS].sort((a, b) => (a.iso < b.iso ? 1 : -1))
  ALL_GENRES = [...new Set(BOOKS.flatMap((b) => b.genres || [b.genre]))]
}

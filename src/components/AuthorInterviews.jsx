  ...
  return (
    <section className="interviews card" id="interviews" ref={root}>
      <div className="interviews-head section-head">
        <h2 className="section-title">Author Interviews</h2>
        <Divider width={300} />
        <p className="interviews-sub">
          In-depth conversations with the brilliant minds behind the stories that inspire us.
        </p>
      </div>
        ...
        {INTERVIEWS.map((item) => (
          <a className="interview-card" key={item.slug} href={interviewHref(item)} aria-label={`${item.title} for ${item.book.title}`}>
            ...
            <div className="interview-body">
              ...
              <h3>{item.title}</h3>
              <p className="interview-book">{item.book.title}</p>
      ...
      <div className="interviews-cta">
        <a className="btn btn-gold-bright btn-interviews" href="/books">
          View All Author Interviews <ArrowRight size={20} />
        </a>
      </div>
    </section>
  )

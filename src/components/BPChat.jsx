...
export default function BPChat() {
  ...
  const speechTag = useMemo(() => LANGS.find((l) => l.code === lang)?.speech || 'en-US', [lang])

  const [msgs, setMsgs] = useState(() => [{ id: nextId(), from: 'bp', lang, ...greeting(lang) }])
  ...
  useEffect(() => {
    if (!langMenu) return
    const onDown = (e) => {
      if (!e.target.closest?.('.bpc-lang')) setLangMenu(false)
    }
  ...
  const tagFor = (code) => LANGS.find((l) => l.code === code)?.speech || 'en-US'

  const say = useCallback((text, code) => {
  ...
  const send = useCallback(
    (raw, forced = null) => {
      ...
      const t = setTimeout(() => {
        ...
        const reply = respond(text, activeLang, forced)
